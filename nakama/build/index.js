...
function InitModule(ctx, logger, nk, initializer) {
    ...
    // Create leaderboard
    nk.leaderboardCreate(LEADERBOARD_ID, true, "descending" /* nkruntime.SortOrder.DESCENDING */, "best" /* nkruntime.Operator.BEST */, "0 0 * * 1");
...
function matchInit(ctx, logger, nk, params) {
    const timedMode = params.timedMode === "true" || params.timedMode === true;
    const state = {
...
function matchLoop(ctx, logger, nk, dispatcher, tick, state, messages) {
    ...
    if (gameState.gameOver) {
        ...
        if (tick - gameState.gameOverTick > tickRate * 10)
            return null;
        return { state: gameState };
    }
...
function updateLeaderboard(nk, logger, state) {
    try {
        for (let i = 0; i < state.playerOrder.length; i++) {
            ...
            const metadata = { wins, losses, draws, streak };
